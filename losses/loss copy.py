...
class VaeLoss(torch.nn.Module):
    def __init__(self, smoothing=0.0, dim=-1, reduction="mean"):
        super().__init__()
        self.celoss = LabelSmoothingLoss(smoothing,dim,reduction)

    def normal_kl(self,p_means_mask, p_logs_mask, q_means_mask, q_logs_mask,lens):

        kl = 0.5*(p_logs_mask - q_logs_mask) + (q_logs_mask.exp() +(q_means_mask - p_means_mask)**2 )/(2*p_logs_mask.exp()) - 0.5
        kl = torch.sum(kl, dim=2)

        mask = generate_length_mask(lens).to(kl.device)
        kl *= mask
        kl = kl.sum() / mask.sum()
        return kl

    def forward(self, output: Dict):
        celoss = self.celoss(output)
        klloss = self.normal_kl(output["q_means"].to(3),output["q_logs"].to(3),output["p_means"].to(3),output["p_logs"].to(3),output["lens"])
