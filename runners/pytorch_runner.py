    ...
    def _forward(self, model, batch, mode, **kwargs):
        ...
        if mode == "train":
            ...
            if len(batch) == 6:
                ...
                kwargs["embedding_lens"] = np.shape(caption_embedding)[-1]
        else:
        ...
        if mode == "train":
            ...
            # pre_captionembedding = outputs_mean
    ...
    def train(self, config, **kwargs):
        ...
        if not conf["distributed"] or not self.local_rank:
            outputdir = Path(conf["outputpath"]) / conf["model"] / \
                f"{self.remark}" / f"seed_{self.seed}"
        ...
        if "embedding_path" in conf:
            if conf["embedding_style"] =='Cosine':
                Similarity = nn.CosineEmbeddingLoss().to(self.device)
            if conf["embedding_style"] =='mse':
                Similarity = nn.MSELoss().to(self.device)
            if conf["embedding_style"] =='mae':
                Similarity = nn.L1Loss().to(self.device)
        #########################
        # Tensorboard record
        #########################
        if not conf["distributed"] or not self.local_rank:
            ...
            with torch.enable_grad(), tqdm(total=len(train_dataloader), ncols=100,
                ...
                for batch in train_dataloader:
                    ...
                    if "embedding_path" in conf:
                        ...
                        if conf["embedding_style"] =='mse':
                            loss = criterion(output["packed_logits"], output["targets"]).to(self.device) + ce_args["dec_par"] * Similarity(output["pre_captionembedding"],torch.Tensor(output["caption_embedding"]).to(self.device))
                        if conf["embedding_style"] =='mae':
                            loss = criterion(output["packed_logits"], output["targets"]).to(self.device) + ce_args["dec_par"] * Similarity(output["pre_captionembedding"],torch.Tensor(output["caption_embedding"]).to(self.device))
                    else:
                        loss = criterion(output["packed_logits"], output["targets"]).to(self.device)
