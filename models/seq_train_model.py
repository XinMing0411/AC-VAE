...
class NScstWrapper(ScstWrapper):
    def __init__(self, model):
        super(NScstWrapper, self).__init__(model)
        self.model = model 
    ...
    def forward(self, *input, **kwargs):
        ...
        if len(input) == 2:
            ...
            output = self.nscst(*input, **kwargs)
        return output
    
    def nscst(self,feats, feat_lens, keys, key2refs, vocabulary, **kwargs):
        output = {}
        self.model.train()
        sample_kwargs = {
            "temperature": kwargs.get("temperature", 1.0),
        ...
        #sampled["seqs"](N,sample_n,seqs)
        reward_score = self.get_critical_reward(sampled["seqs"],
                                                    keys,
                                                    key2refs,
                                                    vocabulary,
                                                    kwargs["scorer"])
        
        output["reward"] = torch.as_tensor(reward_score["reward"]).reshape((-1,))
        output["score"] = torch.as_tensor(reward_score["score"]).reshape((-1,))
