...
class VAEModel(CaptionModel):
    ...
    def forward(self, *input, **kwargs):
            ...
            if len(input) == 4:
                ...
                # print(encoded["audio_embeds"])
                # print(encoded["audio_embeds_lens"])
                ...
                if self.encoder.embed_size!=  self.decoder.embed_size:
                    encoded["audio_embeds"]= self.ln(encoded["audio_embeds"])
                # print(encoded["audio_embeds"].shape)
    ...
    def beam_search(self, encoded, max_length, beam_size):
        ...
        for i in range(encoded["audio_embeds"].size(0)):
            # if i == 0:
            #     embeds = encoded["audio_embeds"]
            # else: 
            #     print(embeds==encoded["audio_embeds"])
            output_i = {}
            # endnodes = []
    ...
    def beamsearch_step(self, decoder_input, encoded, output, i, t, beam_size):
        ...
        # print(decoder_input["enc_mem"].shape)
        # print(decoder_input["word"].shape)
        output_pnet_t = self.pnet(decoder_input["word"],decoder_input["enc_mem"],decoder_input["hiddens_state"],decoder_input["last_z"],decoder_input["enc_mem_lens"])
        decoder_input["z"] = output_pnet_t["z"]
        # print(output_pnet_t["z"])
        output_t = self.decoder(**decoder_input)
        # print(output_t['logits'])
        output_t.update(output_pnet_t)
    ...
    def beamsearch_process(self,  output, output_i, i):
        ...
        # done_beams = output_i["done_beams"]
        # # print(len(done_beams))
        # done_beams = done_beams[:output["seqs"].shape[1]]
        # print(i)    
        # print(output["seqs"].shape)
    ...
    def dbs_process_step(self, output_i, output_t):
        ...
        # output_i["attn_weights"][divm] = output_i["attn_weights"][divm][output_i["prev_words_beam"][divm], :, :]
...
class SimpleVAEModel(CaptionModel):
    ...
    def __init__(self, Audioencoder: nn.Module, Textdecoder: nn.Module, **kwargs):
        ...
        # self.pnet = getattr(text_encoder, kwargs["posterior_model"])(
        #     word_dim = Textdecoder.embed_size,
        #     audiofeats_size = Textdecoder.embed_size,
        #     embed_size = Textdecoder.embed_size,
        #     vocab_size= Textdecoder.vocab_size,
        ...
        # )
        ###GMM
        # self.gmmn_weight = nn.Parameter(torch.ones(kwargs["posterior_args"]["gmm_kernel"], 1, 1, 1).float(), requires_grad=True)
        # self.gmm_kernel = kwargs["posterior_args"]["gmm_kernel"]
        if Textdecoder.embed_size !=Audioencoder.embed_size:
            self.ln = nn.Linear(Audioencoder.embed_size,Textdecoder.embed_size)
            nn.init.xavier_uniform_(self.ln.weight)
    ...
    def decode_step(self,  decoder_input, encoded, caps, output, t, **kwargs):
        ...
        # output_pnet_t = self.pnet(decoder_input["word"],decoder_input["enc_mem"],decoder_input["hiddens_state"],decoder_input["last_z"],decoder_input["enc_mem_lens"])
        # output_pnet_t = {
        #     "mean":torch.zeros(encoded["audio_embeds"].size(0), self.encoder.embed_size).to(encoded["audio_embeds"].device),
        #     "log" : torch.full((encoded["audio_embeds"].size(0),  self.encoder.embed_size),0.5).to(encoded["audio_embeds"].device)
        # }
        # output_pnet_t["z"] = torch.randn(output_pnet_t["mean"].shape)* torch.exp(.5 * output_pnet_t["log"]) + output_pnet_t["mean"]
        #prepare decoder
        if caps is not None:
    ...
    def prepare_decoder_input(self, decoder_input, encoded, caps, output, t, **kwargs):
        ...
        if t == 0:
            ...
            # decoder_input["hiddens_state"] = self.pnet.init_hidden(output["seqs"].size(0),encoded["audio_embeds"].device)
            ####GMM
            # self.gmm_kernel,word.size(0),self.embed_size
            # output["p_means"] = torch.rand((self.gmm_kernel,encoded["audio_embeds"].size(0),self.qnet.embed_size)).to(encoded["audio_embeds"].device)
            # output["p_logs"] = torch.rand((self.gmm_kernel,encoded["audio_embeds"].size(0),self.qnet.embed_size)).to(encoded["audio_embeds"].device)
            # torch.zeros(encoded["audio_embeds"].size(0), self.encoder.embed_size).to(encoded["audio_embeds"].device)
            ...
            output["p_z"] = z
            # gmmn_weight = torch.softmax(self.gmmn_weight, dim=0)
            # output["p_z"] = z.index_select(0, torch.multinomial(gmmn_weight.squeeze(), 1)).squeeze()
            # decoder_input["mean"] = output["p_means"] 
            # decoder_input["log"] =  output["p_logs"]
            decoder_input["z"] =  output["p_z"]
            # print(decoder_input["mean"].shape)
            # "log" : torch.full((encoded["audio_embeds"].size(0),  self.encoder.embed_size),0.5).to(encoded["audio_embeds"].device)
            # decoder_input["last_z"] = torch.zeros(output["seqs"].size(0),self.decoder.embed_size).to(encoded["audio_embeds"].device)
        if t > 0:
            decoder_input["state"] = output["state"]
    ...
    def stepwise_process_step(self, output, output_t, t, sampled):
        ...
        # output["p_means"] = output_t["mean"]
        # output["p_logs"] = output_t["log"]
        # output["p_z"] = output_t["z"]
...
class Hybrid_VAEModel(CaptionModel):
    ...
    def stepwise_forward(self, encoded, caps, cap_lens,**kwargs):
        ...
        if caps is not None:
            ...
            # z = epsilon * torch.exp(.5 * log) + mean
        # self.stepwise_process(output)
        # print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        # print(output["logits"])
        return output
    ...
    def prepare_output(self, encoded, output, max_length):
        ...
        if "q_means" in encoded:
            ...
            # output["q_z_utt"] = encoded["q_z_utt"]
        return output
...
class Hybrid_VAEModel_2(CaptionModel):
    ...
    def __init__(self, Audioencoder: nn.Module, Textdecoder: nn.Module, **kwargs):
        ...
        self.mean_log_out = nn.Linear(Textdecoder.embed_size,Textdecoder.embed_size)
        ...
        self.global_emb = nn.Embedding(Textdecoder.vocab_size, 300)
        self.global_net = text_encoder.Global_encoder()
        
        self.global_to_mu = nn.Linear(2*512, 100)
        self.global_to_logvar = nn.Linear(2*512, 100)
    
    def load_pretrain_global(self,model_path):
        checkpoint = torch.load(model_path, map_location="cpu")
        state_dict = {}

        for key, value in checkpoint["vae_dict"].items():
            if key.startswith("embedding."):
                    model_key = key.replace("embedding.", "global_emb.")
                    state_dict[model_key] = value
            elif  key.startswith("encoder."):
                    model_key = key.replace("encoder.", "global_net.")
                    state_dict[model_key] = value
            elif key.startswith("hidden_to_mu."):
                    model_key = key.replace("hidden_to_mu.", "global_to_mu.")
                    state_dict[model_key] = value
            elif key.startswith("hidden_to_logvar."):
                    model_key = key.replace("hidden_to_logvar.", "global_to_logvar.")
                    state_dict[model_key] = value
        ...
        for param in self.global_emb.parameters():
            param.requires_grad = False
        for param in self.global_to_mu.parameters():
            param.requires_grad = False
        for param in self.global_to_logvar.parameters():
            param.requires_grad = False
    ...
    def stepwise_forward(self, encoded, caps, cap_lens,**kwargs):
        ...
        #     output["p_means_utt"] =  mean_log_out[:,:self.decoder.embed_size]
        #     output["p_logs_utt"] =  mean_log_out[:,self.decoder.embed_size:]
        ...
        # print(output["logits"])
        return output
    def reparameterize_argmax(self,logit, dim=-1):
    ...
    def forward(self, *input, **kwargs):
            ...
            if len(input) == 4:
                ...
                caps_embedding = self.global_emb(caps.long().to(encoded["audio_embeds"].device)[:,1:])#remove <beg> and <end>
                e_hidden = self.global_net(caps_embedding,cap_lens-2)
                mu = self.global_to_mu(e_hidden)
                logvar = self.global_to_logvar(e_hidden)
                encoded["q_means_utt"] = mu
                encoded["q_logs_utt"] = logvar

                output = self.train_forward(encoded, caps, cap_lens, **kwargs)
                preds = self.reparameterize_argmax(output["logits"])
                # print(preds.shape)
                # preds = torch.cat([torch.full((preds.shape[0],1),1).to(preds.device),preds],0)
                seqs_embedding = torch.matmul(preds, self.global_emb.weight)
                e_hidden_p = self.global_net(seqs_embedding,cap_lens-2)
                mu_p = self.global_to_mu(e_hidden_p)
                logvar_p = self.global_to_logvar(e_hidden_p)
                output["p_means_utt"] = mu_p
                output["p_logs_utt"] = logvar_p
            elif len(input) == 2:
                feats, feat_lens = input
                encoded = self.encoder(feats, feat_lens)
                if self.encoder.embed_size!=  self.decoder.embed_size:
